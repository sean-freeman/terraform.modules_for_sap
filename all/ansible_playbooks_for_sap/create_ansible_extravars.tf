...
resource "local_file" "ansible_extravars" {
  filename        = "${path.root}/tmp/ansible_extravars_existing_hosts_generated.yml"
...
${ strcontains(var.module_var_ansible_sap_scenario_selection, "hana") ? "# SAP HANA" : "" }
${ strcontains(var.module_var_ansible_sap_scenario_selection, "hana") ? "# Variable opens firewall for multiple host SAP HANA database" : "" }
${ strcontains(var.module_var_ansible_sap_scenario_selection, "hana") ? "sap_hana_install_update_firewall: true" : "" }


#### Ansible to Terraform - Storage assignment ####
...
${ var.module_var_ibmpower_flag ? "# IBM Power" : "" }
${ var.module_var_ibmpower_flag ? "sap_storage_setup_multipath_enable_and_detect: true" : "" }


#### Ansible to Terraform - Ansible Dictionary for host specifications ####
